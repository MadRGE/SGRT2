  ...
  useEffect(() => {
    ...
    fetch("/api/gestiones", { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then((gestiones: Array<{ expedientes: Expediente[] }>) => {
        const senasa = gestiones.flatMap((g: { expedientes: Expediente[] }) =>
          g.expedientes.filter((e: Expediente) => e.tramiteTipo.organismo.nombre.includes("SENASA") || e.tramiteTipo.organismo.nombre.includes("Sanidad"))
        );
        setExpedientes(senasa);
      });
  }, []);
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-bold">SENASA</h1>
        <span className="text-sm text-text-secondary">RPV · Alimentos · Fitosanitarios · Feed</span>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {["RPV", "Food", "Feed", "Certificaciones"].map((cat) => {
      ...
      <div className="bg-bg-card rounded-xl border border-border">
        <div className="p-4 border-b border-border">
          <h2 className="font-semibold text-sm">Trámites SENASA Activos ({expedientes.length})</h2>
        </div>
        <div className="divide-y divide-border-light">
          ...
          {expedientes.length === 0 && (
            <p className="p-6 text-center text-sm text-text-secondary">Sin trámites SENASA activos</p>
          )}
        </div>
      </div>
    </div>
